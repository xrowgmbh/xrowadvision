...
{/if}
<script type='text/javascript' src="{ezini('AdserverSettings', 'AdserverURL', 'xrowadvision.ini')}/js?wp_id={if and(is_set($block.custom_attributes.zone_override), $block.custom_attributes.zone_override|eq(''))}{$block.custom_attributes.zone_override}{else}{$BannerZones[$block.view]}{/if}"></script>
</div>
{*def $BannerZones = ezini('AdserverSettings', 'BannerZones', 'xrowadvision.ini')}
    <div class="advisionad banner {$block.view}">
